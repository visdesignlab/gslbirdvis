function createLinegraph(svg, path) {
    svg.selectAll("*").remove(); // Clear everything in  SVG
    ...
    d3.json(path).then(data => {
        ...
        const yMax = d3.max(allValues.filter(v => v !== null)); 
        ...
        const line = d3.line()
            ...
            .defined(d => d.value !== null); // Ignore nulls
        ...
        // Create the tooltip div (initially hidden)
        ...
        // Add mouse events to the path element for the tooltip functionality
        path.on("mouseover", function(event, d) {
            // Get the mouse position
            const [mouseX, mouseY] = d3.pointer(event);

            // Get the corresponding year and value (adjust this logic based on your data structure)
            const monthIndex = d3.bisectLeft(years, mouseX); // This might require further adjustments
            const year = years[monthIndex]; // Adjust to get the correct year
            const value = d[monthIndex]; // Adjust to get the correct value

            // Show and position the tooltip
            tooltip.style("display", "block")
                .attr("x", mouseX + 10) // Position tooltip next to mouse
                .attr("y", mouseY - 10)
                .text(`Year: ${year} | Value: ${value}`); // Update text content
        })
